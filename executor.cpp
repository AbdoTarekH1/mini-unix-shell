...
bool execute_builtin(const vector<string>& args) {
    ...
    // Using C++17 to mimic the 'rm -rf' command (Recursive and Force)
    if (args[0] == "rmfolder") {
        if (args.size() < 2) {
            cerr << "rmfolder: missing folder name" << endl;
        } else {
            // I use a std::error_code object here to mimic the '-f' (force) flag.
            // Instead of throwing a fatal C++ exception if a file is locked or missing,
            // it quietly captures the error state so my shell never crashes.
            std::error_code ec;
            
            // std::filesystem::remove_all acts like 'rm -r', recursively traversing 
            // the tree and wiping out all files and sub-directories inside it.
            std::uintmax_t deleted_count = std::filesystem::remove_all(args[1], ec);
            
            if (ec) {
                // If the OS strictly denies the forced deletion (e.g., root permissions needed)
                cerr << "Remove folder error: " << ec.message() << endl;
            } else if (deleted_count > 0) {
                cout << "Successfully force-deleted folder and all contents: " << args[1] << endl;
            } else {
                // True to 'rm -f', it doesn't fail if the folder didn't exist to begin with,
                // but I output a quiet notification so the user isn't confused.
                cout << "rmfolder: target did not exist." << endl;
            }
        }
        return true; 
    }
    ...
    if (args[0] == "help") {
        ...
        cout << "  rmfolder <dir> : Force delete directory recursively" << endl;
...
void execute_standard(vector<string>& args, bool background) {
    ...
    for (size_t i = 0; i < args.size(); ++i) {
        if (args[i] == "<" && i + 1 < args.size()) {
            ...
            clean_args.push_back(args[i]); // Safe copying method
        }
    }
...
// Added the 'bool background' parameter so pipelines can run asynchronously
void execute_pipeline(const vector<vector<string>>& commands, bool background) {
    int num_cmds = commands.size();
    int pipefds[2];
    int in_fd = 0; // Tracks the read end of the previous pipe
    vector<pid_t> pipeline_pids; // Track the PIDs of all processes in the pipeline
    ...
    for (int i = 0; i < num_cmds; ++i) {
        ...
        if (pid < 0) {
            ...
            // --- PARENT PROCESS ---
            pipeline_pids.push_back(pid); // Save the child's PID
    ...
    // --- BACKGROUND LOGIC FOR PIPELINES ---
    if (background) {
        cout << "[Background Pipeline Started] PIDs: ";
        for (pid_t p : pipeline_pids) {
            cout << p << " ";
        }
        cout << endl;
        
        // Push the final job in the pipeline to our global tracker so the 'fg' command can bring it back!
        if (!pipeline_pids.empty()) {
            background_jobs.push_back(pipeline_pids.back());
        }
    } else {
