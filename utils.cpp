...
// Custom string parser. I built a simple state machine here to handle quotation marks.
    ...
    while ((pid = waitpid(-1, &status, WNOHANG)) > 0) {
        ...
        for (auto it = background_jobs.begin(); it != background_jobs.end(); ++it) {
            if (*it == pid) {
                ...
                // --- NEW FIX FOR THE PROMPT ---
                // I print a clean completion message so the user knows it finished
                std::cout << "\n[Background Job " << pid << " Finished]" << std::endl;
                
                // I forcefully tell the GNU Readline library that the screen was 
                // interrupted, and it needs to instantly redraw the colored prompt!
                rl_on_new_line();
                rl_redisplay();
                // ------------------------------
